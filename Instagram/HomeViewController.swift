...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if Auth.auth().currentUser != nil {
            if self.observing == false {
                ...
                postsRef.observe(.childAdded, with: { snapshoto in
                    print("DEBUG_PRINT: .childAddedイベントが発生しました。")
                    
                    // PostDataクラスを生成して受け取ったデータを設定する
                    if let uid = Auth.auth().currentUser?.uid {
                        let postData = PostData(snapshot: snapshoto, myId: uid)
                ...
                postsRef.observe(.childChanged, with: { snapshoto in
                    print("DEBUG_PINT: .childChangedイベントが発生しました。")
            ...
            if observing == true {
                ...
                // DatabaseのobserverEventが上記コードにより解除されたため
                // falseとする
                observing = false
            }
        }
    ...
    func tableView(_ tableView: UITableView, estimatedHeightForRowAt indexPath: IndexPath) -> CGFloat {
        // Auto Layoutを使ってセルの高さを自動的に変更する
        return UITableViewAutomaticDimension
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        // セルをタップされたら何もせずに状態を解除する
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        postCommentViewController.postData = sender as! PostData
    }
